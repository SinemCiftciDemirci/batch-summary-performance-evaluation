...
def process_files(model_folder, gpt_folder, cos_folder, output_dir="metrics"):
    ...
    # Progress bar for files in the model folder
    model_files = [f for f in os.listdir(model_folder) if f.endswith(".txt")]
    with tqdm(total=len(model_files), desc="Processing Files", unit="file") as pbar:
        for model_file in model_files:
            ...
            if not gpt_match or not cos_match:
                print(f"Matching files not found: {model_file}")
                pbar.update(1)
                continue
    ...
    print(f"Performance table saved as '{excel_path}'")
